...
class diff_match_patch {
  ...
  * {Diff(Operation.DIFF_DELETE, "Hello"), Diff(Operation.DIFF_INSERT, "Goodbye"),
  ...
  class Diff {
   public:
    ...
    static string_t strOperation(Operation op) {
      switch (op) {
        case DIFF_INSERT:
          return traits::cs(L"DIFF_INSERT");
        case DIFF_DELETE:
          return traits::cs(L"DIFF_DELETE");
        case DIFF_EQUAL:
 ...
 private:
  static void diff_main(const string_t &text1, const string_t &text2, bool checklines, clock_t deadline, Diffs& diffs) {
    ...
    if (text1 == text2) {
      if (!text1.empty()) {
        diffs.push_back(Diff(DIFF_EQUAL, text1));
      }
    }
    else {
      ...
      if (!commonprefix.empty()) {
        diffs.push_front(Diff(DIFF_EQUAL, commonprefix));
      }
      if (!commonsuffix.empty()) {
        diffs.push_back(Diff(DIFF_EQUAL, commonsuffix));
      }
 ...
 private:
  static void diff_compute(string_t text1, string_t text2, bool checklines, clock_t deadline, Diffs& diffs) {
    if (text1.empty()) {
      ...
      diffs.push_back(Diff(DIFF_INSERT, text2));
      return;
    }

    if (text2.empty()) {
      // Just delete some text (speedup).
      diffs.push_back(Diff(DIFF_DELETE, text1));
      return;
    }

    {
      ...
      if (i != string_t::npos) {
        ...
        const Operation op = (text1.length() > text2.length()) ? DIFF_DELETE : DIFF_INSERT;
        diffs.push_back(Diff(op, longtext.substr(0, i)));
        diffs.push_back(Diff(DIFF_EQUAL, shorttext));
    ...
    if (deadline != std::numeric_limits<clock_t>::max()) {
      ...
      if (diff_halfMatch(text1, text2, hm)) {
        ...
        diffs.push_back(Diff(DIFF_EQUAL, hm.mid_common));
        Diffs diffs_b;
 ...
 private:
  static void diff_lineMode(string_t text1, string_t text2, clock_t deadline, Diffs& diffs) {
    ...
    diffs.push_back(Diff(DIFF_EQUAL, string_t()));
    int count_delete = 0;
    int count_insert = 0;
 ...
 public:
  static void diff_cleanupSemantic(Diffs &diffs) {
    ...
    for (cur_diff = diffs.begin(); cur_diff != diffs.end();) {
      if ((*cur_diff).operation == DIFF_EQUAL) {
        // Equality found.
        ...
        if ((*cur_diff).operation == DIFF_INSERT) {
        ...
        if (!lastequality.empty()
          ...
          (*(cur_diff = equalities.back())).operation = DIFF_INSERT;
          // Duplicate record.
          diffs.insert(cur_diff, Diff(DIFF_DELETE, lastequality));
    ...
    if ((cur_diff = diffs.begin()) != diffs.end()) {
      for (typename Diffs::iterator prev_diff = cur_diff; ++cur_diff != diffs.end(); prev_diff = cur_diff) {
        if ((*prev_diff).operation == DIFF_DELETE &&
            (*cur_diff).operation == DIFF_INSERT) {
          ...
          if (overlap_length != 0) {
            ...
            diffs.insert(cur_diff, Diff(DIFF_EQUAL, insertion.substr(0, overlap_length)));
            (*prev_diff).text =
 ...
 public:
  static void diff_cleanupSemanticLossless(Diffs &diffs) {
    ...
    for (typename Diffs::iterator next_diff = cur_diff; ++next_diff != diffs.end(); prev_diff = cur_diff, cur_diff = next_diff) {
      if ((*prev_diff).operation == DIFF_EQUAL &&
        (*next_diff).operation == DIFF_EQUAL) {
  ...
  void diff_cleanupEfficiency(Diffs &diffs) const {
    ...
    for (typename Diffs::iterator cur_diff = diffs.begin(); cur_diff != diffs.end();) {
      if ((*cur_diff).operation == DIFF_EQUAL) {
        ...
        // An insertion or deletion.
        if ((*cur_diff).operation == DIFF_DELETE) {
          post_del = true;
        } else {
          post_ins = true;
        }
 ...
 public:
  static void diff_cleanupMerge(Diffs &diffs) {
    diffs.push_back(Diff(DIFF_EQUAL, string_t()));  // Add a dummy entry at the end.
        ...
        case DIFF_EQUAL:
          if (count_delete + count_insert > 1) {
            ...
            if (count_delete != 0 && count_insert != 0) {
              ...
              if (commonlength != 0) {
                if (cur_diff != diffs.begin()) {
                  ...
                  diffs.insert(cur_diff, Diff(DIFF_EQUAL, text_insert.substr(0, commonlength)));
                }
            ...
            if (!text_delete.empty()) {
              diffs.insert(cur_diff, Diff(DIFF_DELETE, text_delete));
            }
            if (!text_insert.empty()) {
              diffs.insert(cur_diff, Diff(DIFF_INSERT, text_insert));
            }
 ...
 public:
  static int diff_xIndex(const Diffs &diffs, int loc) {
    ...
    if (last_diff != diffs.end() && (*last_diff).operation == DIFF_DELETE) {
    ...
    for (typename string_t::const_pointer token = delta.c_str(); token - delta.c_str() < (int)delta.length(); token += token_len + 1) {
      ...
      switch (traits::to_wchar(*token)) {
        case L'+':
          ...
          diffs.push_back(Diff(DIFF_INSERT, param));
          break;
        case L'-':
          // Fall through.
        case L'=': {
          ...
          if (traits::to_wchar(*token) == L'=') {
            diffs.push_back(Diff(DIFF_EQUAL, text));
          } else {
            diffs.push_back(Diff(DIFF_DELETE, text));
          }
          break;
        }
        default:
 ...
 protected:
  void patch_addContext(Patch &patch, const string_t &text) const {
    ...
    if (!prefix.empty()) {
      patch.diffs.push_front(Diff(DIFF_EQUAL, prefix));
    }
    // Add the suffix.
    ...
    if (!suffix.empty()) {
      patch.diffs.push_back(Diff(DIFF_EQUAL, suffix));
    }
 ...
 public:
  Patches patch_make(const string_t &text1, const Diffs &diffs) const {
    ...
    if (!diffs.empty()) { // Get rid of the null case.
      ...
      for (typename Diffs::const_iterator cur_diff = diffs.begin(); cur_diff != diffs.end(); ++cur_diff) {
        if (patch.diffs.empty() && (*cur_diff).operation != DIFF_EQUAL) {
          ...
          if ((int)text1.length() > Match_MaxBits
            ...
            for (typename Diffs::const_iterator cur_diff = (*cur_patch).diffs.begin(); cur_diff != (*cur_patch).diffs.end(); ++cur_diff) {
              if ((*cur_diff).operation != DIFF_EQUAL) {
                ...
                } else if ((*cur_diff).operation == DIFF_DELETE) {
                  // Deletion
 ...
 public:
  string_t patch_addPadding(Patches &patches) const {
    ...
    if (firstPatchDiffs.empty() || firstPatchDiffs.front().operation != DIFF_EQUAL) {
      // Add nullPadding equality.
      firstPatchDiffs.push_front(Diff(DIFF_EQUAL, nullPadding));
    ...
    if (lastPatchDiffs.empty() || lastPatchDiffs.back().operation != DIFF_EQUAL) {
      // Add nullPadding equality.
      lastPatchDiffs.push_back(Diff(DIFF_EQUAL, nullPadding));
    ...
    for (typename Patches::iterator cur_patch = patches.begin(); cur_patch != patches.end();) {
      ...
      while (!bigpatch.diffs.empty()) {
        ...
        if (!precontext.empty()) {
          ...
          patch.diffs.push_back(Diff(DIFF_EQUAL, precontext));
        }
        while (!bigpatch.diffs.empty()
          ...
          if (diff_type == DIFF_INSERT) {
            ...
            empty = false;
          } else if (diff_type == DIFF_DELETE && patch.diffs.size() == 1
              && patch.diffs.front().operation == DIFF_EQUAL
        ...
        if (!postcontext.empty()) {
          ...
          if (!patch.diffs.empty()
              && patch.diffs.back().operation == DIFF_EQUAL) {
            patch.diffs.back().text += postcontext;
          } else {
            patch.diffs.push_back(Diff(DIFF_EQUAL, postcontext));
          }
        }
        if (!empty) {
      ...
      while (text - textline.c_str() < (int)textline.length()) {
        ...
        for (text += text_len + 1; text - textline.c_str() < (int)textline.length(); text += text_len + 1) {
          ...
          switch (traits::to_wchar(sign)) {
            case L'-':
              // Deletion.
              patch.diffs.push_back(Diff(DIFF_DELETE, line));
              continue;
            case L'+':
              // Insertion.
              patch.diffs.push_back(Diff(DIFF_INSERT, line));
              continue;
            case L' ':
              // Minor equality.
              patch.diffs.push_back(Diff(DIFF_EQUAL, line));
              continue;
            case L'@':
              // Start of next patch.
              break;
